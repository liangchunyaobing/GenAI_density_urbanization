...
class MyDataset(Dataset):
    def __init__(self, image_dir, data_dir, hint_dir):
        ...
        for f in self.data_dir: 
            ...
            #df = pd.concat([df, dfi[(dfi['Train']==1)&(dfi['img']==1)&(dfi['withBuilding']==1)]])
    ...
    def __getitem__(self, item):
        ...
        hint_name = self.image_dir + f'{city}_4lu_Stage_1_rt{str(rt)}_{r}_{d}/{str(xtile)}_{str(ytile)}_{r}_{d}.tif'
        img_name =  self.image_dir + f'{city}_4lu_Stage_2_rt{str(rt)}_{r}_{d}/{str(xtile)}_{str(ytile)}_{r}_{d}.tif'
